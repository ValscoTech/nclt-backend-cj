...
export const fetchPartyDetails = async (req, res) => {
  const client = createClient();
  ...
  try {
    ...
    return res.status(200).json({
      filingNo,
      header: modal.header,
      currentDetails: modal.currentDetails,
      firstPartyDetail: modal.partyDetails[0] ?? null,
      partyDetails: modal.partyDetails,
      applicationDetails: modal.applicationDetails,
      disposedDetails: modal.disposedDetails,
      proceedingDetails: modal.proceedingDetails,
      nclatDetails: modal.nclatDetails,
    });
  } catch (err) {
    return res.status(500).json({
      error: "Internal server error",
